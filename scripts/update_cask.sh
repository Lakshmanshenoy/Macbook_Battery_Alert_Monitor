...
# Assume cask is at Casks/battery-alert.rb or Formula/battery-alert.rb
if [ -f "Casks/battery-alert.rb" ]; then
  CASK_PATH="Casks/battery-alert.rb"
elif [ -f "Formula/battery-alert.rb" ]; then
  CASK_PATH="Formula/battery-alert.rb"
else
...
git add "$CASK_PATH"
git commit -m "bump(cask): battery-alert $TAG"
git push origin "$BRANCH"

# Create PR using GitHub CLI if available
if command -v gh >/dev/null 2>&1; then
  gh pr create --title "bump(cask): battery-alert $TAG" --body "Automated cask bump for $TAG" --base main --head "$BRANCH"
else
